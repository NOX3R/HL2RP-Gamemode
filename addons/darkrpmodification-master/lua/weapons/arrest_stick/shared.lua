...
if CLIENT then
	SWEP.PrintName = "Baton d'arrestation"
	SWEP.Slot = 1
	SWEP.SlotPos = 3
end
...
SWEP.Instructions = "Clic gauche pour arrêter\nClic droit pour changer de batons"
	...
	if not jpc or jpc == 0 then
		...
		if ent:IsNPC() then
			...
			if not ent.Babygod then
			    ...
			    RunConsoleCommand("_FAdmin", "freeze", ent:UserID(), 10)
				umsg.Start("Openjail",self.Owner)
				umsg.End()
