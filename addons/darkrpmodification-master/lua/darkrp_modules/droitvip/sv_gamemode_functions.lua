...
function GM:PlayerSpawnProp(ply, model)
	...
	local allowed = ((GAMEMODE.Config.propspawning or (FAdmin and FAdmin.Access.PlayerHasPrivilege(ply, "rp_prop")) or ply:IsAdmin()) or string.find(ply:GetUserGroup() , "VIP") and true) or false
...
function GM:PlayerInitialSpawn(ply)
	...
	DarkRP.log(ply:Nick().." ("..ply:SteamID()..") s'est connecté", Color(0, 130, 255))
...
function GM:PlayerSpawn(ply)
	...
	if ply:Team() == TEAM_VORT then pk_pills.apply(ply,"vort") ply:SetNoDraw(true)end
	if ply:Team() == TEAM_VORT_ESCLAVE then pk_pills.apply(ply,"vort_slave") ply:SetNoDraw(true)end
	if ply:Team() == TEAM_VORT_SCIEN then pk_pills.apply(ply,"ep2_vort_scientist")ply:SetNoDraw(true) end
	if ply:Team() == TEAM_HOBO then pk_pills.apply(ply,"zombie") ply:SetNoDraw(true) end
	if ply:Team() == TEAM_HOBOF then pk_pills.apply(ply,"zombie_fast") ply:SetNoDraw(true) end
	if ply:Team() == TEAM_STALKER then pk_pills.apply(ply,"stalker") ply:SetNoDraw(true)end	
	if ply:Team() == TEAM_HOBOH then pk_pills.apply(ply,"ep1_zombine") ply:SetNoDraw(true)end
	if ply:Team() == TEAM_HOBOZ then pk_pills.apply(ply,"zombie_poison")ply:SetNoDraw(true) end
	if ply:Team() == TEAM_HEAD then pk_pills.apply(ply,"headcrab") end
	if ply:Team() == TEAM_HEADF then pk_pills.apply(ply,"headcrab_fast") end
	if ply:Team() == TEAM_HEADZ then pk_pills.apply(ply,"headcrab_poison") end
	if ply:Team() == TEAM_SCN then pk_pills.apply(ply,"cityscanner")ply:SetNoDraw(true) ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_SCN_MINE then pk_pills.apply(ply,"clawscanner") ply:SetNoDraw(true) ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_ANTLION then pk_pills.apply(ply,"antlion") ply:SetNoDraw(true) end
	if ply:Team() == TEAM_ANTLION_WORK then pk_pills.apply(ply,"ep2_antlion_worker") ply:SetNoDraw(true) end
	if ply:Team() == TEAM_POLICE_05 then   ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_POLICE_04 then   ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_POLICE_03 then   ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_POLICE_02 then   ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_POLICE_01 then  ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_CHIEF then  ply:AllowFlashlight(true)end
	if ply:Team() == TEAM_POLICEC then  ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_POLICE_GHOST then  ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_OTA then  ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_OTA_PRISON then  ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_OTA_KING then  ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_REB_ELITE then  ply:AllowFlashlight(true) end
	if ply:Team() == TEAM_REB_ELITEF then  ply:AllowFlashlight(true) end
...
/*---------------------------------------------------------------------------
Remove with a delay if the player doesnt rejoin before the timer has run out
---------------------------------------------------------------------------*/
